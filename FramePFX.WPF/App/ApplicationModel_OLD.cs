...
namespace FramePFX.WPF.App {
    ...
    /// </summary>
    public class ApplicationModel_OLD : IApplication_OLD {
        ...
        public ApplicationModel_OLD(AppWPF app) {
            ...
            this.RegisterService<IDispatcher>(new DispatcherDelegate(this.dispatcher));
        }

        static ApplicationModel_OLD() {
        ...
        public static void OnDisposeToken(OperationToken token) {
            ...
            if (type == 0) {
                ((ApplicationModel_OLD) token.app).FinishWriteOperation();
            }
            else if (type == 1 && !hasPreExistingReadAccess) {
                ((ApplicationModel_OLD) token.app).FinishReadOperation();
            }
        }
