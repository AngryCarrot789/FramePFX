...
public class RenderManager {
    ...
    public double AverageVideoRenderTimeMillis { get; private set; }
    ...
    private void BeginRender(TimeSpan frame) {
#if DEBUG
        ...
        if (frame.Ticks < 0 || frame >= this.Timeline.MaximumDuration)
    ...
    /// </summary>
    /// <param name="location">The frame to render</param>
    public Task RenderTimelineAsync(TimeSpan location, CancellationToken token, RenderQuality quality = RenderQuality.Medium) {
        Project? project = this.Timeline.Project;
        if (project == null) {
            return Task.CompletedTask;
        }

        this.BeginRender(location);
        long beginRenderTicks;
        ...
        try {
            ...
            beginRenderTicks = Time.GetSystemTicks();
            ...
            for (int i = this.Timeline.Tracks.Count - 1; i >= 0; i--) {
                ...
                if (track is VideoTrack videoTrack && videoTrack.IsVisible) {
                    if (videoTrack.PrepareRenderFrame(imgInfo, location, quality)) {
        ...
        Task renderVideo = Task.Run(async () => {
            ...
            this.AverageVideoRenderTimeMillis = (Time.GetSystemTicks() - beginRenderTicks) / (double) TimeSpan.TicksPerMillisecond;
        }, token);

        return renderVideo.ContinueWith(t => {
    ...
    public static int BeginClipOpacityLayer(SKCanvas canvas, VideoClip clip, ref SKPaint? paint) {
        if (clip.UsesCustomOpacityCalculation || clip.InternalRenderOpacity >= 1.0) {
    ...
    public void InvalidateRender() {
        ...
        VideoEditor? editor = this.Timeline.Project?.VideoEditor;
    ...
    private async Task DoScheduledRender() {
        VideoEditor? editor;
        if (this.suspendRenderCount < 1 && (editor = this.Timeline.Project?.VideoEditor) != null) {
            try {
                await this.RenderTimelineAsync(this.Timeline.PlayHeadLocation, CancellationToken.None, RenderQuality.Low);
            }
